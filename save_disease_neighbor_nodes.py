...
list_disease_names = get_diseases() # used to save the knowledge
# list_disease_names = get_unsaved_diseases() # there are a lot of disease experiments have the same name as disease knowledge
# index = list_disease_names.index('urinary system benign neoplasm')
for node_value in list_disease_names:
    print(f'node_value = {node_value}')
    df = get_context_using_spoke_api(node_value)
    if df.empty == False:
        df.to_csv(f'./data/graph/graph_{node_value}.csv')
        save_df_neo4j(df)
